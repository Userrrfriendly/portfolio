...
const MyStory = () => {
    ...
    const p7 = `After weighing my options, I narrowed down my focus to two coding paths that appealed to me: the first one was creating custom tools/extensions upon already existing software (preferably C# with .NET framework for ArcGIS or Python for qGIS) and the second option was to create dynamic web maps/webGIS applications that would serve engineering data or even offer complete web-GIS solutions (although web mapping sounds rather simplistic – especially now that we live in the age of Google Earth and Google maps, serving engineering maps on the web is a complex problem: most mapping applications that engineers use have different projection and coordinate systems, also they need to serve custom imagery either satellite or orthophotos, vector formats that are not web friendly at all (.shp I am looking at you) plus they should offer editing tools and possibly even some spatial analysis tools. Well obviously since you are reading this either in your browser or in your phone its pretty obvious that I choose the web mapping path.`;
    const p9 = `So I did some research and found out that the most straightforward way to display maps on the web was with Leaflet.js - a JavaScript mapping library. So I had to start by learning some JavaScript, also since the map would be displayed in the browser I had to get some familiarity with HTML and CSS. Finally I had a concrete goal in front of me: in order to create custom web-mapping applications I had to learn JavaScript, HTML and CSS.`;
    ...
    return (
        <main className="main-story">
            ...
            <p className="story-p" aria-label={p2}> <Ft>{p2}</Ft></p>
            <p className="story-p" aria-label={p3}> <Ft>{p3}</Ft></p>
            <ul>
                <li
            ...
            <p className="story-p" aria-label={p4}> <Ft>{p4}</Ft></p>
            <p className="story-p" aria-label={p5}> <Ft>{p5}</Ft></p>
            ...
            <p className="story-p" aria-label={p6}> <Ft>{p6}</Ft></p>
            ...
            <p className="story-p" aria-label={p9}><Ft>{p9}</Ft></p>
            <p className="story-p" aria-label={p10}><Ft>{p10}</Ft></p>
            <p className="story-p" aria-label={p11}><Ft>{p11}</Ft></p>
            <p className="story-p" aria-label={p12}><Ft>{p12}</Ft></p>
            <p className="story-p" aria-label={p13}><Ft>{p13}</Ft></p>
            <p className="story-p" aria-label={p14}><Ft>{p14}</Ft></p>
            <p className="story-p" aria-label={p15}><Ft>{p15}</Ft></p>
